...
export class MailgunService {
  ...
  constructor(private readonly configService: ConfigService) {
    this.domain =
      this.configService.get<string>("MAILGUN_DOMAIN") || "onlypark.com.au";
    this.apiKey = this.configService.get<string>("MAILGUN_SECRET") || "";

    if (!this.apiKey) {
      this.logger.warn(
        "MAILGUN_SECRET not configured, email sending will be simulated",
      );
    }
  ...
  async send(request: SendEmailRequest): Promise<SendEmailResponse> {
    ...
    try {
      if (!this.apiKey || !this.mailgun) {
        ...
        this.logger.log(
          `[SIMULATED] Sending email to ${request.to}: ${request.subject}`,
        );
        this.logger.log(
          `[SIMULATED] From: ${request.senderName || "OnlyPark"} <${request.from || this.configService.get("EMAIL_FROM")}>`,
        );
        this.logger.log(
          `[SIMULATED] Body: ${request.body.substring(0, 200)}...`,
        );
      ...
      this.logger.error(
        `Failed to send email to ${request.to}: ${error.message}`,
      );
