...
export class EmailNotificationService {
    ...
    async sendUsingTemplate<T>(
        ...
        try {
            const template = await this.templateStorageService.get(request.templateKey);
            ...
            if (!template.titleTemplate) {
                throw new Error(`Template missing titleTemplate: ${request.templateKey}`);
            }

            if (!template.bodyTemplate) {
                throw new Error(`Template missing bodyTemplate: ${request.templateKey}`);
            }
            ...
            if (template.layout) {
                ...
                if (!templateLayout.bodyTemplate) {
                    throw new Error(`Layout template missing bodyTemplate: ${template.layout}`);
                }
