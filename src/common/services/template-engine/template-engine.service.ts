...
export interface ITemplateEngine {
    compile<T>(template: string, data: { [K in keyof T]: string }): Promise<string>;
    compileFromFile<T>(templatePath: string, data: { [K in keyof T]: string }): Promise<string>;
}
...
export class TemplateEngineService implements ITemplateEngine {
    ...
    async compileFromFile<T>(
        ...
        try {
            const fullPath = path.join(process.cwd(), 'assets', 'templates', 'html', templatePath);
