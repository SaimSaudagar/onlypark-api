...
export class OutstandingRegistrationService {
  ...
  async create(createDto: any): Promise<OutstandingRegistration> {
    ...
    const savedEntity =
      await this.outstandingRegistrationRepository.create(entity);
    return savedEntity as unknown as OutstandingRegistration;
  }

  async findAll(
    options?: FindManyOptions<OutstandingRegistration>,
  ): Promise<OutstandingRegistration[]> {
    return await this.outstandingRegistrationRepository.find(options);
  }

  async findOne(
    options?: FindOneOptions<OutstandingRegistration>,
  ): Promise<OutstandingRegistration> {
    return await this.outstandingRegistrationRepository.findOne(options);
  }

  async update(id: string, updateDto: any) {
    const entity = await this.outstandingRegistrationRepository.findOne({
      where: { id },
    });
    if (entity) {
