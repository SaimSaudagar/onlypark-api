...
export class SubCarParkService {
  ...
  async create(
    ...
    const eventExpiryDateObj =
      event && eventExpiryDate ? new Date(eventExpiryDate) : null;
    ...
    try {
      const masterCarPark =
        await this.masterCarParkService.findById(masterCarParkId);
      ...
      const existingSubCarPark = await queryRunner.manager
        ...
        .andWhere("subCarPark.masterCarParkId = :masterCarParkId", {
          masterCarParkId,
        })
        .getOne();
      ...
      if (whitelistCompanies && whitelistCompanies.length > 0) {
        const whitelistCompaniesWithSubCarPark = whitelistCompanies.map(
          (company) => {
            return {
      ...
      return {
        ...
        tenancies: savedTenancies.map((tenant) => ({
          id: tenant.id,
          tenantName: tenant.tenantName,
          tenantEmail: tenant.tenantEmail,
        })),
        whitelistCompanies: savedWhitelistCompanies.map((company) => ({
          id: company.id,
          companyName: company.companyName,
          domainName: company.domainName,
        })),
  ...
  async findAll(
    ...
    const [subCarParks, totalItems] =
      await this.subCarParkRepository.findAndCount({
        skip,
        take,
        relations: {
          masterCarPark: true,
    ...
    const response: FindAllSubCarParkResponse[] = subCarParks.map(
      (subCarPark) => ({
        id: subCarPark.id,
  ...
  async findOne(
    ...
    return {
      ...
      tenancies:
        subCarPark.tenancies?.map((tenancy) => ({
          id: tenancy.id,
          tenantName: tenancy.tenantName,
          tenantEmail: tenancy.tenantEmail,
        })) || [],
      whitelistCompanies:
        subCarPark.whitelistCompanies?.map((company) => ({
          id: company.id,
          companyName: company.companyName,
          domainName: company.domainName,
        })) || [],
    ...
    try {
      ...
      if (tenancies !== undefined) {
        ...
        if (!hasTenancyIds) {
          ...
          const providedTenancyIds = tenancies
            .filter((t) => t.id)
            .map((t) => t.id);
        ...
        for (const tenancy of tenancies) {
          if (tenancy.id) {
            ...
            const existingTenancy = await queryRunner.manager.findOne(Tenancy, {
              where: { id: tenancy.id },
            });
            if (existingTenancy) {
      ...
      if (whitelistCompanies !== undefined) {
        ...
        if (!hasCompanyIds) {
          ...
          if (
            subCarPark.whitelistCompanies &&
            subCarPark.whitelistCompanies.length > 0
          ) {
          ...
          const providedCompanyIds = whitelistCompanies
            .filter((c) => c.id)
            .map((c) => c.id);
        ...
        for (const company of whitelistCompanies) {
          if (company.id) {
            ...
            if (existingCompany) {
              ...
              const domainExists = await queryRunner.manager.findOne(
                WhitelistCompany,
                {
                  where: {
                    domainName: company.domainName,
      ...
      return {
        ...
        tenancies:
          subCarParkWithRelations?.tenancies?.map((tenant) => ({
            id: tenant.id,
            tenantName: tenant.tenantName,
            tenantEmail: tenant.tenantEmail,
          })) || [],
        whitelistCompanies:
          subCarParkWithRelations?.whitelistCompanies?.map((company) => ({
            id: company.id,
            companyName: company.companyName,
            domainName: company.domainName,
          })) || [],
