...
import { CreateVisitorRequest, CreateVisitorResponse, FindVisitorRequest, FindVisitorResponse, UpdateVisitorRequest, UpdateVisitorResponse, GetAssignedSubCarParksResponse } from './visitor.dto';
...
export class VisitorService extends BaseService {
    ...
    async create(request: CreateVisitorRequest): Promise<CreateVisitorResponse> {
        const { registrationNumber, email, subCarParkId, tenancyId, startDate, endDate, status } = request;
    ...
    async findAll(request: FindVisitorRequest): Promise<ApiGetBaseResponse<FindVisitorResponse>> {
        const { search, dateFrom, dateTo, status, sortField, sortOrder, pageNo, pageSize } = request;
        ...
        const [visitorBookings, totalItems] = await this.visitorBookingRepository.findAndCount(query);
    ...
    async findOne(options: FindOneOptions<VisitorBooking>): Promise<VisitorBooking> {
    ...
    async update(id: string, request: UpdateVisitorRequest): Promise<UpdateVisitorResponse> {
        const entity = await this.visitorBookingRepository.findOne({ where: { id } });
        if (!entity) {
            throw new CustomException(
                ErrorCode.VISITOR_BOOKING_NOT_FOUND.key,
                HttpStatus.BAD_REQUEST,
    ...
    async getAssignedSubCarParks(): Promise<GetAssignedSubCarParksResponse[]> {
        ...
        return carparkManager.carparkManagerVisitorSubCarParks?.map(assignment => ({
