...
export class SubCarParkService extends BaseService {
  ...
  async findAll(
    ...
    const [subCarParks, totalItems] =
      await this.subCarParkRepository.findAndCount(query);

    // Map the results to response format
    const response: FindSubCarParkResponse[] = subCarParks.map(
      (subCarPark) => ({
        id: subCarPark.id,
  ...
  async findByAssignmentType(
    request: FindSubCarParkRequest,
    assignmentType: "visitor" | "whitelist" | "blacklist",
  ): Promise<ApiGetBaseResponse<FindSubCarParkResponse>> {
    const { pageNo, pageSize, sortField, sortOrder, search, status } = request;
    const skip = (pageNo - 1) * pageSize;
    const take = pageSize;
    ...
    const assignedSubCarParkIds =
      await this.getAssignedSubCarParksByType(assignmentType);
  ...
  private async getAssignedSubCarParksByType(
    assignmentType: "visitor" | "whitelist" | "blacklist",
  ): Promise<string[]> {
    const userId = this.authenticatedUser.id;
    ...
    switch (assignmentType) {
      case "visitor":
        carparkManager.carparkManagerVisitorSubCarParks?.forEach(
          (assignment) => {
            if (assignment.subCarPark?.id) {
              assignedSubCarParkIds.add(assignment.subCarPark.id);
            }
        ...
        break;
      case "whitelist":
        carparkManager.carparkManagerWhitelistSubCarParks?.forEach(
          (assignment) => {
            if (assignment.subCarPark?.id) {
              assignedSubCarParkIds.add(assignment.subCarPark.id);
            }
        ...
        break;
      case "blacklist":
        carparkManager.carparkManagerBlacklistSubCarParks?.forEach(
          (assignment) => {
            if (assignment.subCarPark?.id) {
              assignedSubCarParkIds.add(assignment.subCarPark.id);
            }
