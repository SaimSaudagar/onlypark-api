...
import { GetInfringementPaymentRequest, GetInfringementPaymentResponse } from './infringement.dto';
...
export class InfringementService {
  ...
  async getInfringementPayment(request: GetInfringementPaymentRequest): Promise<GetInfringementPaymentResponse> {
    ...
    // Convert status to lowercase with hyphen format
    const statusFormatted = infringement.status.toLowerCase().replace('_', '-');
    ...
    response.status = statusFormatted;
